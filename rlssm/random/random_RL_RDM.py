...
def simulate_rlrdm_2A(task_design,
    ...
    if nonlinear_mapping:
        data['cor_drift'] = (gen_drift_scaling + gen_drift_asym*(data['Q_mean_t'] - data['Q_min']))/(1+np.exp(gen_slop*(data['Q_mean_t']+data['Q_max_t']-2*data['Q_cor'])))
        data['inc_drift'] = (gen_drift_scaling + gen_drift_asym*(data['Q_mean_t'] - data['Q_min']))/(1+np.exp(gen_slop*(data['Q_mean_t']+data['Q_max_t']-2*data['Q_inc']))) 
    else:
...
def simulate_hier_rlrdm_2A(task_design,
    ...
    if (type(gen_mu_alpha) == float) | (type(gen_mu_alpha) == int):
        ...
        data = pd.concat([data.set_index('participant'),
                          parameters], axis=1, ignore_index=False).reset_index().rename(columns={'index': 'participant'})
