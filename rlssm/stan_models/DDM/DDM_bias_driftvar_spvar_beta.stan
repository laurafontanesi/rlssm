data {
	...
	array[N] real beta;									// regression coefficients from neuroimaging
...
transformed parameters {
	...
	for (n in 1:N) {
		drift_t[n] = drift_trial_mu + z_drift_trial[n]*drift_trial_sd + drift_coefficient*beta[n];
		drift_ll[n] = drift_t[n]*accuracy[n];
		threshold_t[n] = transf_threshold;
		ndt_t[n] = transf_ndt;
		rel_sp_t[n] = Phi(rel_sp_trial_mu + z_rel_sp_trial[n]*rel_sp_trial_sd + rel_sp_coefficient*beta[n]);
