...
transformed parameters {
  ...
  for (n in 1:N) {
    ...
    if(Q[cor_option[n]] < Q[inc_option[n]]){
      Q_min_t = Q[cor_option[n]];
      Q_max_t = Q[inc_option[n]];
    }
    else{
      Q_max_t = Q[cor_option[n]];
      Q_min_t = Q[inc_option[n]];
    }
    ...
    if (Q_min_t != Q_max_t){  
      ...
      drift_cor_t[n] = (drift_scaling_sbj[participant[n]] + 0.1*drift_asym_sbj[participant[n]]*(Q_mean_pres[n] - Q_min)) / (1+exp(0));
      drift_inc_t[n] = (drift_scaling_sbj[participant[n]] + 0.1*drift_asym_sbj[participant[n]]*(Q_mean_pres[n] - Q_min)) / (1+exp(0));
    }
